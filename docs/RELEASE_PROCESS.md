...
6. Run `CASCADELENS_PYTHON=/path/to/python3.11+ npm run release:verify -- release/vX.Y.Z`. It inspects ZIP/TAR entry, expansion, compression-ratio, path, type, duplicate, and nested-archive budgets before extraction; then it extracts without `.git`, executes the Python source tests and demo verification, runs `npm ci`, regenerates deterministic artifacts, executes full web CI, performs two additional offline builds, and requires the rebuilt `dist` digest to match the manifest.
