    ...
    <PageShell>
      ...
      <section className="page-content method-layout">
        ...
        <article className="prose method-prose">
          ...
          <section id="cascade"><Eyebrow>03 · Cascade</Eyebrow><h2>Bounded daily propagation over a typed directed graph.</h2><p>The built-in engine activates and retires shocks from their declared time intervals, transmits impact over eligible dependency edges, accounts for node buffers, and calculates every requested horizon independently. Results distinguish the time-weighted mean, within-horizon peak, and final-day impact. Flow-share sums above one are reported rather than silently normalized.</p><pre><code>impactₜ(v) = 1 − Π(1 − contributionᵢ,ₜ){"\n"}contributionᵢ,ₜ = impactₜ₋₁(u) × edge_weight × transmission</code></pre></section>
          <section id="interventions"><Eyebrow>04 · Interventions</Eyebrow><h2>Feasibility first, Pareto frontier second.</h2><p>Up to sixteen interventions are exhaustively enumerated. Budget, lead time, count, units, and mutual exclusion are checked before evaluation. The product retains the do-nothing baseline and reports cost–risk non-dominated bundles plus reversal thresholds.</p></section>
          ...
          <section id="benchmark"><Eyebrow>06 · Benchmark</Eyebrow><h2>Outcomes live in a separate partition.</h2><p>Post-event observations cannot support graph inputs or shocks. Scoring is blocked on leakage. With valid separated outcomes, the benchmark reports error, rank, direction, interval coverage and width, empirical coverage calibration error, and regret versus an explicit zero-impact baseline. With no comparable outcome—or for a synthetic stress—the result is explicitly scenario-only.</p></section>
          <section id="riskpack"><Eyebrow>07 · RiskPack</Eyebrow><h2>Integrity is necessary and still not validation.</h2><p>A RiskPack holds the scenario, sealed graph, source manifest, assumptions, model card, cascade bounds, intervention and observability outputs, benchmark status, limitations, portable rebuild command, and relative SHA-256 checksums. Verification proves internal integrity, not predictive validity or adoption.</p></section>
        </article>
      </section>
    </PageShell>
  );
